  ...
  <center><h2 class="text-center">LAPORAN ABSENSI SISWA</h2></center>
    ...
    <tbody>
      ...
      $sql = mysqli_query($koneksi, "SELECT tbl_siswa.nis, tbl_siswa.nama, tbl_pembayaran_spp.tgl_pembayaran,       tbl_pembayaran_spp.total
                    ...
                    JOIN tbl_siswa ON tbl_siswa.id = tbl_pembayaran_spp.id_siswa WHERE tbl_siswa.deleted=0 ORDER BY tbl_pembayaran_spp.id DESC");
      foreach ($sql as $data) {
      ?>
        <tr>
          <td data-header="NIS"><?= $data['nis']; ?></td>
          <td data-header="Nama"><?= $data['nama']; ?></td>
          <td data-header="TanggalBayar" style="text-align: center;"><?= date('d/m/Y', strtotime($data['tgl_pembayaran'])); ?></td>
